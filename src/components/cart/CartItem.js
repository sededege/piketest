...
const CartItem = ({ item, setFlag, flag }) => {
  ...
  return (
    <div className="w-full p-4 px-2 rounded-lg shadow-md flex items-center gap-2">
      <img
        src={item && item[0].item.thumbnail}
      ...
      <div className="flex flex-col gap-2">
        <p className="text-base text-[0.8rem] text-gray-500">
          {item[0].item.name}
        </p>
      ...
      <div className="group flex flex-col items-center  ml-auto cursor-pointer">
        <div className="flex gap-2">
          <motion.div
            className="bg-pike  p-1 items-center rounded-lg"
            whileTap={{ scale: 0.75 }}
        ...
        <div className="flex mt-8">
          <p className="text-sm block text-gray-800 font-semibold">
            $ {parseFloat(item[0].item.price) * qty}
          </p>
        </div>
      </div>
    </div>
